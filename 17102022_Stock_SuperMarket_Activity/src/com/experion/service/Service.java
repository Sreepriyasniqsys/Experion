...
public class Service {
	...
	public void displayStock(Stock[] stock)
	{
		...
		for(int i =0;i>3;i++)
		{
		System.out.println(stock[i].getStockCode()+ "   " + stock[i].getStockName() + "  " + stock[i].getStockDescription()+ "  " + stock[i].getStockPrice() + "    "
				+ stock[i].getOpenDate() +"   " + stock[i].getValidityDate()+ "   " + stock[i].getExpiryDate()+ "  " + stock[i].isActive());
	}
	}
}
